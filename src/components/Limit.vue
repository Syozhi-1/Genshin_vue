...
export default {
  ...
  // 获取json数据
  created() {
    this.$http.get("static/limit.json").then((res) => {
      // console.log(res.data);
      this.kachi = res.data;
      if (this.kachi.hasOwnProperty("up")) {
        var r4 = this.kachi.up["r4"]["1"].toString();
        var r5 = this.kachi.up["r5"]["1"].toString();
      ...
      switch (p) {
        ...
        case 4:
          ...
          if ((Math.round(Math.random() * 10) % 2 == 0 ) && this.kachi.hasOwnProperty("up")) {
            // up
            chi = this.kachi.up["r4"]["1"]; // 抽奖改为只抽up
            len = chi.length - 1;
        ...
        default:
          ...
          var chi = this.kachi.r5["1"];
          var len = chi.length - 1;
          var huo = this.kachi.r5["1"][Math.round(Math.random() * len)];
          up = "否";

          // 判断是否抽中up角色
          if ((Math.round(Math.random() * 10) % 2 == 0 ) && this.kachi.hasOwnProperty("up")) {
            // up
            chi = this.kachi.up["r5"]["1"]; // 抽奖改为只抽up
            len = chi.length - 1;
            if (len == 0) {
              huo = chi[len];
            } else {
